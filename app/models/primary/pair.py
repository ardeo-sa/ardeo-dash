...
nl_filter_org_code : str
    Optional filter code to associate the pair with a specific organisation.
pair_first : str
    The first element of the pair, typically used as a key.
pair_second : str
    The second element of the pair, typically used as a value.
namedList_id : int
    Foreign key linking to the NamedList model, indicating the list to which the pair belongs.
child_index : int
    Index used for ordering pairs within the named list.

Relationships:
-------------
named_list :
    Relationship with the NamedList model to access list details and maintain pair associations.
"""
